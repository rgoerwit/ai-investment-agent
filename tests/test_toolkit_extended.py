...
class TestSentimentEdgeCases:
    ...
    async def test_sentiment_empty_response(self, mock_stocktwits):
        ...
        mock_stocktwits.get_sentiment.return_value = {"total_messages": 0, "messages": []}
...
class TestCrossToolFailures:
    ...
    async def test_all_tools_fail_gracefully(self, mock_fetcher, mock_tavily, mock_stocktwits):
        """Test that if all data sources fail, system doesn't crash."""
        mock_fetcher.get_financial_metrics = AsyncMock(side_effect=Exception("FMP Down"))
