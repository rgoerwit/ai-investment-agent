...
class TestNormalizedEarningsRequiredFlag:
    ...
    def test_cautionary_event_check_is_not_treated_as_an_assertion(self):
        flags, _ = RedFlagDetector.detect_red_flags(
            _sparse_metrics(
                "MRQ growth is extreme; verify if cyclical peak/trough, "
                "one-time gain/loss, or data error."
            ),
            "6782.TW",
        )
