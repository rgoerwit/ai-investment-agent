        ...
        with patch("src.memory.config") as mock_config:
            ...
            with patch("src.memory.GoogleGenerativeAIEmbeddings") as mock_emb:
                with patch("chromadb.PersistentClient") as mock_client:
                    ...
                    mock_emb_instance.aembed_query = AsyncMock(
                        return_value=embedding_with_nan
                    )
                    mock_emb.return_value = mock_emb_instance
                    ...
                    result = await memory.add_situations(
                        ["Test with NaN embedding"], [{"ticker": "NAN"}]
                    )
        ...
        with patch("src.memory.config") as mock_config:
            ...
            with patch("src.memory.GoogleGenerativeAIEmbeddings") as mock_emb:
                with patch("chromadb.PersistentClient") as mock_client:
                    ...
                    mock_emb_instance.aembed_query = AsyncMock(
                        return_value=embedding_with_inf
                    )
                    mock_emb.return_value = mock_emb_instance
                    ...
                    result = await memory.add_situations(
                        ["Test with inf embedding"], [{"ticker": "INF"}]
                    )
        ...
        with patch("src.memory.config") as mock_config:
            ...
            with patch("src.memory.GoogleGenerativeAIEmbeddings") as mock_emb:
                with patch("chromadb.PersistentClient") as mock_client:
                    ...
                    result = await memory.add_situations([""], [{"ticker": "EMPTY"}])
...
class TestMemoryIsolationEdgeCases:
    ...
    async def test_cleanup_with_locked_collection(self):
        ...
        with patch("chromadb.PersistentClient") as mock_client:
            ...
            mock_client.return_value.delete_collection.side_effect = Exception(
                "Collection locked"
            )
        ...
        with patch("src.memory.config") as mock_config:
            ...
            with patch("src.memory.GoogleGenerativeAIEmbeddings") as mock_emb:
                with patch("chromadb.PersistentClient") as mock_client:
                    ...
                    results = await memory.query_similar_situations(
                        "test", metadata_filter={"invalid": {"nested": "bad"}}
                    )
                    assert results == []
